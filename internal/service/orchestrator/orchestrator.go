...
func AskAnAgent(args map[string]any) map[string]any {
	...
	case "coder":
		analysisForCoder, ok := args["analysis"].(string)
		if !ok {
			return map[string]any{
...
func (p *AgentOrchestrator) RequestAgent(contents []map[string]any) map[string]any {
	...
	request := map[string]any{
		...
		"tools": []map[string]any{
			{"functionDeclarations": GetOrchestratorToolsMap()}, // Also fixed snake_case to camelCase
		},
	}
...
- execution: Executes planned tasks`,
		Parameters: repository.Parameters{
			...
			Properties: map[string]*repository.Properties{
				...
				"analysis": {
					Type:        repository.TypeString,
					Description: "The Analysis which you got from analysis agent keep it simple and detailed with each and every analysis and bullet points",
				},
			},
